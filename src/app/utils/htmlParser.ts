...
export const parseAboutUsContent = (htmlContent: string) => {
  ...
  const whatDoesGreenTitle =
    whatDoesGreenElement?.textContent || "What does GREEN do";

  // Extract subtitle for "What does GREEN do" section
  const whatDoesGreenSubtitle =
    tempDiv.querySelector("h3 em")?.textContent || "";
...
export const parseWhyGreenContent = (htmlContent: string) => {
  ...
  const envisionTitle =
    sectionTitleElements[1]?.textContent || "Envision and Enlighten lives with";
  const solutionsTitle =
    sectionTitleElements[2]?.textContent ||
