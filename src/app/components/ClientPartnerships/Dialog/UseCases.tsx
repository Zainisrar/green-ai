...
const defaultItems = [
  {
    img: { alt: "Nurse at clinic", src: "/images/client-partnerships/nurse-review.png" },
  ...
  {
    img: { alt: "Parent with children", src: "/images/client-partnerships/rural-review.png" },
  ...
  {
    img: { alt: "Local technician", src: "/images/client-partnerships/incubator-review.png" },
    title: '"I Was Trained By GREEN. Now I Earn As An O&M Technician And Support My Family."',
    reference: "– Incubator Graduate, Madang",
  },
  {
    img: { alt: "Village elder", src: "/images/client-partnerships/elder-review.png" },
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {items.map((item, idx) => (
          <div key={idx} className="overflow-hidden">
            <img src={item.img.src} alt={item.img.alt} className="h-48 w-full object-cover" />
            <div className="pt-4">
              <p className="mb-3 text-sm font-medium italic leading-relaxed text-gray-800">
                {item.title}
              </p>
              <p className="text-xs font-semibold text-[#4CAF50]">{item.reference}</p>
            </div>
          </div>
        ))}
      </div>
