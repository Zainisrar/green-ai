...
const defaultItems = [
  { title: "Needs-Based Engineering", description: "Every solution designed around mission-critical needs" },
  { title: "Delivery Discipline", description: "Timelines. Compliance. Zero-excuse execution" },
  { title: "Transparency & Reporting", description: "Real-time monitoring, impact dashboards, partner visibility" },
  { title: "Post-Project Continuity", description: "Training, O&M, warranty coverage, GRID-INTEL™ support" },
];
...
const OurClientPartnershipModel = ({ isOpen, onClose, data }: Props) => {
  ...
  return (
    <ClientInfoModal isOpen={isOpen} onClose={onClose}>
      ...
      <div className="mb-6">
        <p className="text-base font-medium text-gray-700 sm:text-lg">{description}</p>
      </div>

      <div className="flex flex-col items-start gap-8 lg:flex-row lg:gap-12">
        <div className="flex-1 space-y-5">
          {items.map((item, idx) => (
            <div key={idx} className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
              <div className="text-base font-bold text-gray-800 sm:text-lg">{item.title}</div>
              <div className="text-sm text-gray-700 sm:text-base">- {item.description}</div>
            </div>
          ))}
        </div>
