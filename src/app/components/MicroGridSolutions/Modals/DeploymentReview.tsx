...
const DeploymentReview = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.Code === "001") {
        setSuccessMessage(
          data.Message || "Your deployment review request has been submitted successfully!",
        );
        resetForm();
        setTimeout(() => {
          onClose();
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          />
          <label htmlFor="deployment-agree" className="text-sm text-gray-700 sm:text-base">
