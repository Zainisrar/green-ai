...
const DeploymentReview = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Hybrid & Microgrid Solutions — Book a Deployment Review",
      `Organization: ${formData.organization}`,
      `Project / deployment name: ${formData.deploymentName}`,
    ...
    try {
      ...
      if (data.Code === "001") {
        setSuccessMessage(
          data.Message ||
            "Your deployment review request has been submitted successfully!",
        );
        resetForm();
        setTimeout(() => {
          onClose();
  ...
  return (
    <ProductEnquiryFrame
      labelledBy="deployment-review-title"
      onClose={onClose}
      closeLabel="Close deployment review dialog"
    >
      <div className={styles.content}>
        <header className={styles.dialogHeader}>
          <h2 id="deployment-review-title">BOOK A DEPLOYMENT REVIEW</h2>
        </header>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <div className={`${styles.agreement} ${styles.row5}`}>
            ...
            />
            <label htmlFor="deployment-agree">
