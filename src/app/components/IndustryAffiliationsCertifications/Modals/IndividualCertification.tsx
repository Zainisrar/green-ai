...
const IndividualCertification = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Individual Certification / Verification Letter (Industry Affiliations & Certifications)",
      `Organization: ${formData.organization}`,
      `Type of request: ${formData.requestType}`,
      `Preferred consultation type: ${formData.consultationType}`,
      `Details / purpose of request: ${formData.details || "None"}`,
    ].join("\n");
    ...
    <EngineeringFormModal
      ...
      title={
        <>
          ...
          <span className="text-green-600">VERIFICATION LETTER</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          ...
          >
            ...
            <option value="individual-certification">Individual Certification</option>
        ...
        <textarea
          name="details"
          placeholder="DETAILS / PURPOSE OF REQUEST"
        ...
        <div className="flex items-start gap-3">
          <input
            type="checkbox"
            id="individual-cert-agree"
            checked={agreed}
            onChange={(e) => setAgreed(e.target.checked)}
            className="mt-1 h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500"
          />
          <label htmlFor="individual-cert-agree" className="text-sm text-gray-700 sm:text-base">
