...
const JoinTheNetwork = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Women in Energy — Join the Network",
      `Organization: ${formData.organization}`,
      `Area of interest: ${formData.areaOfInterest}`,
      `Role / expertise: ${formData.roleExpertise}`,
      `Brief introduction: ${formData.introduction}`,
    ].join("\n");
    ...
    try {
      ...
      if (data.Code === "001") {
        setSuccessMessage(
          data.Message || "You've successfully joined the network!",
        );
        resetForm();
        setTimeout(() => {
          onClose();
  ...
  return (
    <EngineeringFormModal
      ...
      title={
        <>
          JOIN THE <span className="text-green-600">NETWORK</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          ...
          <select
            name="roleExpertise"
            value={formData.roleExpertise}
            onChange={handleInputChange}
            className={`${formFieldClass} cursor-pointer ${
              formData.roleExpertise ? "text-gray-700" : "text-gray-500"
            }`}
            required
          >
            <option value="">YOUR ROLE / EXPERTISE</option>
        ...
        <div className="flex items-start gap-3">
          <input
            type="checkbox"
            id="jointhenetwork-agree"
            checked={agreed}
            onChange={(e) => setAgreed(e.target.checked)}
            className="mt-1 h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500"
          />
          <label htmlFor="jointhenetwork-agree" className="text-sm text-gray-700 sm:text-base">
