...
const TechnicalDeepDive = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.Code === "001") {
        setSuccessMessage(
          data.Message || "Your technical deep-dive request has been submitted successfully!",
        );
        resetForm();
        setTimeout(() => {
          onClose();
  ...
  return (
    <EngineeringFormModal
      ...
      title={
        <>
          SCHEDULE A <span className="text-green-600">TECHNICAL DEEP-DIVE</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          <select
            ...
            className={`${formFieldClass} cursor-pointer ${
              formData.focusArea ? "text-gray-700" : "text-gray-500"
            }`}
            required
          >
            <option value="">FOCUS AREA</option>
            <option value="grid-modeling">Grid Modeling &amp; Simulation</option>
            <option value="forecasting">Forecasting &amp; Analytics</option>
        ...
        <div className="flex items-start gap-3">
          ...
          />
          <label htmlFor="deepdive-agree" className="text-sm text-gray-700 sm:text-base">
