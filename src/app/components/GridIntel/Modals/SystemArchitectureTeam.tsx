...
const SystemArchitectureTeam = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Grid Intel — Engage Our System Architecture Team",
  ...
  return (
    <ProductEnquiryFrame
      labelledBy="grid-intel-team-title"
      onClose={onClose}
      closeLabel="Close system architecture team dialog"
    >
      <div className={styles.content}>
        <header className={styles.dialogHeader}>
          <h2 id="grid-intel-team-title">
            ENGAGE OUR <strong>SYSTEM ARCHITECTURE TEAM</strong>
          </h2>
        </header>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <div className={`${styles.agreement} ${styles.row5}`}>
            <input
              type="checkbox"
              id="architecture-agree"
              checked={agreed}
              onChange={(e) => setAgreed(e.target.checked)}
            />
            <label htmlFor="architecture-agree">
          ...
          <div className={`${styles.row} ${styles.row6}`}>
            ...
            >
              <span>{isLoading ? "Submitting..." : "Engage Team"}</span>
            </button>
          </div>
        </form>
      </div>
...
export default SystemArchitectureTeam;
