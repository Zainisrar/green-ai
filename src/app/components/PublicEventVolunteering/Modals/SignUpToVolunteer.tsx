...
const SignUpToVolunteer = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Public Events & Volunteering — Sign Up to Volunteer",
    ...
    try {
      ...
      if (data.Code === "001") {
        setSuccessMessage(
          data.Message || "Thanks for signing up to volunteer! We'll be in touch soon.",
        );
        resetForm();
        setTimeout(() => {
          onClose();
  ...
  return (
    <EngineeringFormModal
      ...
      title={
        <>
          SIGN UP TO <span className="text-green-600">VOLUNTEER</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          <select
            ...
            onChange={handleInputChange}
            className={`${formFieldClass} cursor-pointer ${
              formData.ageGroup ? "text-gray-700" : "text-gray-500"
            }`}
            required
          >
          ...
          >
            ...
            <option value="solar-literacy">Solar Literacy & Awareness</option>
            <option value="school-programs">School Programs</option>
            <option value="rural-demonstrations">Rural Demonstrations</option>
        ...
        <div className={formGridClass}>
          ...
          <select
            ...
            onChange={handleInputChange}
            className={`${formFieldClass} cursor-pointer ${
              formData.availability ? "text-gray-700" : "text-gray-500"
            }`}
            required
          >
            ...
            <option value="one-off-events">One-off Events</option>
          </select>
        </div>
        ...
        <div className="flex items-start gap-3">
          <input
            type="checkbox"
            id="signuptovolunteer-agree"
            checked={agreed}
            onChange={(e) => setAgreed(e.target.checked)}
            className="mt-1 h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500"
          />
          <label htmlFor="signuptovolunteer-agree" className="text-sm text-gray-700 sm:text-base">
...
export default SignUpToVolunteer;
