...
const SubmitInterest = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Submit Interest (Our Procurement Philosophy)",
    ...
    try {
      ...
      if (data.Code === "001") {
        setSuccessMessage(
          data.Message || "Your interest has been submitted successfully!",
        );
        resetForm();
        setTimeout(() => {
          onClose();
        ...
        setErrorMessage(data.Message || "Failed to submit your interest. Please try again.");
      }
    } catch (error) {
      setErrorMessage(
  ...
  return (
    <EngineeringFormModal
      ...
      title={
        <>
          SUBMIT <span className="text-green-600">INTEREST</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          ...
          >
            ...
            <option value="supply-partnership">Supply Partnership</option>
        ...
        <div className="flex items-start gap-3">
          <input
            type="checkbox"
            id="submit-interest-agree"
            checked={agreed}
            onChange={(e) => setAgreed(e.target.checked)}
            className="mt-1 h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500"
          />
          <label htmlFor="submit-interest-agree" className="text-sm text-gray-700 sm:text-base">
