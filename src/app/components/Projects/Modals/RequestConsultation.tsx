...
const RequestConsultation = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Request a Consultation (Flagship Projects)",
      `Service needed: ${formData.service}`,
      `Message / project details: ${formData.details || "None"}`,
    ].join("\n");
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          ...
          >
            ...
            <option value="om-monitoring">O&amp;M &amp; Monitoring</option>
        ...
        <div className="flex items-start gap-3">
          <input
            type="checkbox"
            id="flagship-consult-agree"
            checked={agreed}
            onChange={(e) => setAgreed(e.target.checked)}
            className="mt-1 h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500"
          />
          <label htmlFor="flagship-consult-agree" className="text-sm text-gray-700 sm:text-base">
