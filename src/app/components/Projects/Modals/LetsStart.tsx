...
const LetsStart = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Let's Start (Project Portfolio)",
      `Project type: ${formData.projectType}`,
      `Brief project details: ${formData.details || "None"}`,
    ].join("\n");
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          ...
          <select
            ...
            className={`${formFieldClass} cursor-pointer ${
              formData.projectType ? "text-gray-700" : "text-gray-500"
            }`}
            required
          >
        ...
        <div className="flex items-start gap-3">
          <input
            type="checkbox"
            id="project-letsstart-agree"
            checked={agreed}
            onChange={(e) => setAgreed(e.target.checked)}
            className="mt-1 h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500"
          />
          <label htmlFor="project-letsstart-agree" className="text-sm text-gray-700 sm:text-base">
