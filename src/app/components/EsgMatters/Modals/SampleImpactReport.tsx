...
const SampleImpactReport = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Sample Impact Report or M&E Framework (Impact Measurement & ESG)",
      `Organization: ${formData.organization}`,
      `Request type: ${formData.requestType}`,
      `Preferred consultation type: ${formData.consultationType}`,
      `Brief purpose / use case: ${formData.purpose || "None"}`,
    ].join("\n");
  ...
  return (
    <EngineeringFormModal
      ...
      title={
        <>
          SAMPLE IMPACT REPORT OR{" "}
          <span className="text-green-600">M&amp;E FRAMEWORK</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          ...
          >
            ...
            <option value="sample-impact-report">Sample Impact Report</option>
            <option value="me-framework">M&amp;E Framework</option>
            <option value="esg-policy-brief">ESG Policy Brief</option>
        ...
        <textarea
          name="purpose"
          placeholder="BRIEF PURPOSE / USE CASE"
        ...
        <div className="flex items-start gap-3">
          <input
            type="checkbox"
            id="sample-report-agree"
            checked={agreed}
            onChange={(e) => setAgreed(e.target.checked)}
            className="mt-1 h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500"
          />
          <label htmlFor="sample-report-agree" className="text-sm text-gray-700 sm:text-base">
...
export default SampleImpactReport;
