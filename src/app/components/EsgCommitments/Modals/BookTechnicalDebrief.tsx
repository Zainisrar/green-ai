...
const BookTechnicalDebrief = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Book a Technical Debrief (Sustainability & ESG Commitments)",
      `Organization: ${formData.organization}`,
      `What they'd like to review: ${formData.reviewTopic}`,
      `Preferred session type: ${formData.sessionType}`,
    ...
    try {
      ...
      if (data.Code === "001") {
        setSuccessMessage(
          data.Message || "Your technical debrief request has been submitted successfully!",
        );
        resetForm();
        setTimeout(() => {
          onClose();
  ...
  return (
    <EngineeringFormModal
      ...
      title={
        <>
          BOOK A <span className="text-green-600">TECHNICAL DEBRIEF</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          ...
          <select
            name="reviewTopic"
            value={formData.reviewTopic}
            onChange={handleInputChange}
            className={`${formFieldClass} cursor-pointer ${
              formData.reviewTopic ? "text-gray-700" : "text-gray-500"
            }`}
            required
          >
            <option value="">WHAT WOULD YOU LIKE TO REVIEW?</option>
            <option value="environmental">Environmental Stewardship</option>
            <option value="social">Social Responsibility</option>
            <option value="governance">Ethical Governance</option>
            <option value="esg-reporting">ESG Reporting &amp; KPIs</option>
        ...
        <div className={formGridClass}>
          ...
          <select
            ...
            onChange={handleInputChange}
            className={`${formFieldClass} cursor-pointer ${
              formData.sessionType ? "text-gray-700" : "text-gray-500"
            }`}
            required
          >
        ...
        <div className="flex items-start gap-3">
          ...
          />
          <label htmlFor="debrief-agree" className="text-sm text-gray-700 sm:text-base">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:justify-end sm:gap-6">
          ...
          >
            <span className="block text-sm font-bold text-gray-900 sm:text-base">
              {isLoading ? "Submitting..." : "Book Session"}
            </span>
          </button>
        </div>
      </form>
