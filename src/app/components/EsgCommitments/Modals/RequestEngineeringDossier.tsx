...
const RequestEngineeringDossier = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = [
      "Request our Engineering Dossier (Sustainability & ESG Commitments)",
    ...
    try {
      ...
      if (data.Code === "001") {
        setSuccessMessage(
          data.Message || "Your dossier request has been submitted successfully!",
        );
        resetForm();
        setTimeout(() => {
          onClose();
  ...
  return (
    <EngineeringFormModal
      ...
      title={
        <>
          REQUEST OUR <span className="text-green-600">ENGINEERING DOSSIER</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          ...
          <select
            ...
            className={`${formFieldClass} cursor-pointer ${
              formData.purpose ? "text-gray-700" : "text-gray-500"
            }`}
            required
          >
            ...
            <option value="investment">Investment Evaluation</option>
        ...
        <div className="flex items-start gap-3">
          ...
          />
          <label htmlFor="dossier-agree" className="text-sm text-gray-700 sm:text-base">
