...
const UploadPhotoVideo = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fileName) {
      setErrorMessage("Please upload a photo or video file.");
      return;
    }
    ...
    const message = [
      "Community Voices — Upload a Photo / Video",
      `Organization: ${formData.organization}`,
      `Project / program name: ${formData.projectName}`,
      `Title / caption: ${formData.caption}`,
    ...
    try {
      ...
      if (data.Code === "001") {
        setSuccessMessage(
          data.Message || "Your photo / video has been submitted successfully!",
        );
        resetForm();
        setTimeout(() => {
          onClose();
  ...
  return (
    <EngineeringFormModal
      ...
      title={
        <>
          UPLOAD A <span className="text-green-600">PHOTO / VIDEO</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-[#23B14D]">
            (Formats: Jpeg, Png, MP4/PDF, Size: Below 2Mb)
          </p>
        </div>
