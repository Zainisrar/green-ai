...
const StorageSystemReview = ({ isOpen, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.Code === "001") {
        setSuccessMessage(
          data.Message || "Your storage system review request has been submitted successfully!",
        );
        resetForm();
        setTimeout(() => {
          onClose();
  ...
  return (
    <EngineeringFormModal
      ...
      title={
        <>
          BOOK A <span className="text-green-600">STORAGE SYSTEM REVIEW</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          />
          <label htmlFor="storage-agree" className="text-sm text-gray-700 sm:text-base">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:justify-end sm:gap-6">
          ...
          >
            <span className="block text-sm font-bold text-gray-900 sm:text-base">
              {isLoading ? "Submitting..." : "Book Review"}
            </span>
          </button>
        </div>
      </form>
