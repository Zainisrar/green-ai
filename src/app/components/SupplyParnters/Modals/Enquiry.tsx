...
const Enquiry = ({ isOpen, onClose }: Props) => {
  ...
  return (
    <EngineeringFormModal isOpen={isOpen} onClose={onClose} title={<>ENQUIRY</>}>
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          <select
            ...
            className={`${formFieldClass} cursor-pointer ${
              formData.productInterest ? "text-gray-700" : "text-gray-500"
            }`}
            required
          >
        ...
        <div className="flex items-start gap-3">
          <input
            type="checkbox"
            id="supply-enquiry-agree"
            checked={agreed}
            onChange={(e) => setAgreed(e.target.checked)}
            className="mt-1 h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500"
          />
          <label htmlFor="supply-enquiry-agree" className="text-sm text-gray-700 sm:text-base">
