  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.Code === "001") {
        ...
        setErrorMessage(data.Message || "Failed to submit your proposal. Please try again.");
      }
    } catch (error) {
      setErrorMessage(
  ...
  return (
    <EngineeringFormModal
      ...
      title={
        <>
          SUBMIT PROPOSAL /{" "}
          <span className="text-green-600">COLLABORATION INQUIRY</span>
        </>
      }
    >
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className={formGridClass}>
          <select
            ...
            onChange={handleInputChange}
            className={`${formFieldClass} cursor-pointer ${
              formData.proposalType ? "text-gray-700" : "text-gray-500"
            }`}
            required
          >
        ...
        <div className="flex items-start gap-3">
          ...
          />
          <label htmlFor="proposal-agree" className="text-sm text-gray-700 sm:text-base">
