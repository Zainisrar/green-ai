...
async function fetchDynamicSlugs(): Promise<string[]> {
  ...
  const apiFetches: Array<{ pattern: string; url: string; slugKey?: string }> =
    [
      {
        pattern: "/home/[slug]",
        url: "https://greencms.percepco.co.uk/api/home",
        slugKey: "slug",
      },
      {
        pattern: "/expertise/[slug]",
        url: "https://greencms.percepco.co.uk/api/expertise",
      },
      {
        pattern: "/engineering/products/[slug]",
        url: "https://greencms.percepco.co.uk/api/engineering/products",
      },
      {
        pattern: "/enlighten/insights-articles/[slug]",
        url: "https://greencms.percepco.co.uk/api/enlighten/insights-articles",
      },
      {
        pattern: "/insights/[slug]",
        url: "https://greencms.percepco.co.uk/api/insights",
      },
    ];
  ...
  for (const { pattern, url, slugKey } of apiFetches) {
    try {
      ...
      for (const item of items.slice(0, 5)) {
        ...
        if (!rawSlug) continue;
        const slug = String(rawSlug).startsWith("/")
          ? String(rawSlug)
          : `${prefix}/${rawSlug}`;
...
async function auditPage(page: Page, url: string) {
  ...
  if (!response) {
    addBug({
      ...
      url,
      description:
        "Navigation timed out or threw before a response was received.",
...
test.describe("GreenAI Application Audit", () => {
  ...
  test("homepage hero and navigation", async ({ page }) => {
    ...
    const logo = page
      .locator(
        'img[alt="logo"], img[alt="GREEN logo"], img[alt="GREEN"], img[alt*="GREEN"], a[href="/"] img',
      )
      .first();
    await expect(logo)
    ...
    const navLinks = page
      .locator("a")
      .filter({ hasText: /Explore|Energy|Expertise|Enlist|Engage|Elements/i });
  ...
  test("navigation modal opens", async ({ page }) => {
    ...
    if (await menuTrigger.isVisible()) {
      ...
      const navPanel = page
        .locator("button")
        .filter({ hasText: /Explore|Energy|Evolution|Ecosystem/i });
      const visible = await navPanel
        .first()
        .isVisible()
        .catch(() => false);
      if (!visible) {
        addBug({
          ...
          url: "/",
          description:
            "Clicking the menu icon does not reveal navigation sections.",
  ...
  test("broken internal nav links in ProductNavigation", async ({ page }) => {
    ...
    for (const { route } of brokenRoutes) {
      const res = await page
        .goto(`${BASE_URL}${route}`, { waitUntil: "domcontentloaded" })
        .catch(() => null);
  ...
  test.afterAll(async () => {
    ...
    fs.writeFileSync(
      outPath,
      formatBugReport(deduped, collectStaticRoutes().length),
      "utf-8",
    );
...
function formatBugReport(items: BugReport[], routeCount: number): string {
  ...
  if (items.length === 0) {
    ...
    lines.push("");
    lines.push(
      "All audited routes loaded successfully with no console errors, broken images, or visible runtime failures.",
    );
  }
